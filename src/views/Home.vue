<template>
  ...
  /* ============================================
     ...
     ============================================ */
  .app-container {
    /* ⚠️ 关键调整：如果还遮挡，请把这个数字改大！(尝试 60px, 70px, 80px) */
    --tabbar-height: 20px; 
    
    /* 手机底部安全区 (iPhone 底部横条，通常 20px-34px) */
    --safe-area-bottom: env(safe-area-inset-bottom);
    
    /* 总预留高度 = Tabbar 高度 + 安全区 + 额外缓冲 (10px 防止贴太紧) */
    --bottom-offset: calc(var(--tabbar-height) + var(--safe-area-bottom) + 10px);
  }
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
  ...
  .app-container { 
    ...
    position: relative; 
    background: #f5f7fa; 
    /* 可选：给整个页面加底部 padding，防止地图内容也被遮挡 */
    padding-bottom: var(--bottom-offset);
  }
  
  /* ... (中间其他基础样式保持不变，如 debug-btn, overlay 等) ... */
  ...
  .location-btn { 
    position: absolute; 
    /* ✅ 原有 100px + 新的 offset (确保浮得更高) */
    bottom: calc(100px + var(--tabbar-height) + var(--safe-area-bottom)); 
    right: 20px; 
    z-index: 920; 
    width: 44px; 
    height: 44px; 
  ...
  .location-toast { 
    position: absolute; 
    /* ✅ 原有 150px + 新的 offset */
    bottom: calc(150px + var(--tabbar-height) + var(--safe-area-bottom)); 
    right: 20px; 
    z-index: 930; 
  ...
  .suggestion-icon { font-size: 14px; margin-top: 1px; }
  .suggestion-content { flex: 1; overflow: hidden; }
  .suggestion-name { font-weight: 600; color: #333; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .suggestion-address { font-size: 11px; color: #999; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-top: 2px; }
  .suggestion-empty { padding: 8px 10px; color: #999; text-align: center; }
  .input-active { border-color: #007AFF; border-bottom-left-radius: 0; border-bottom-right-radius: 0; }
  ...
  @media (max-height: 700px) { 
    .location-btn { 
      bottom: calc(80px + var(--tabbar-height) + var(--safe-area-bottom)); 
    } 
    .location-toast { 
      bottom: calc(130px + var(--tabbar-height) + var(--safe-area-bottom)); 
    } 
  }
  </style>